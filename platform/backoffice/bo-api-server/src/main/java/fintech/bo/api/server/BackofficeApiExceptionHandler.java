...
@Slf4j
@ControllerAdvice(basePackages = "fintech.bo.api")
@ResponseBody
public class BackofficeApiExceptionHandler {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public BackofficeApiError handle(ConstraintViolationException e) {
        Set<ConstraintViolation<?>> violations = e.getConstraintViolations();
        if (violations.isEmpty()) {
            return new BackofficeApiError(String.format("Error: %s", e.getMessage()));
        }
        return new BackofficeApiError(String.format("Error: %s", violations.iterator().next().getMessage()));
    }
}
