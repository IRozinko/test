...
@Component
public class LastLoanApplicationCloseReasonPredicate implements PredicateResolver {
    ...
    public Predicate resolve(MarketingConditionContext context) {
        ...
        return JPAExpressions.selectFrom(loanApplication).where(
            loanApplication.id.eq(
                JPAExpressions.select(la.id.max()).from(la).where(la.clientId.eq(context.targetClient().id).and(la.closeDate.isNotNull()))
            ).and(loanApplication.closeReason.equalsIgnoreCase(value))
        ).exists();
    }
}
