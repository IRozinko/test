...
public class ExpireActivity implements TaskListener {
    ...
    public void handle(TaskContext context) {
        ...
        log.info("Task [{}] of activity [{}] expired with resolution [{}] by agent [{}]", task.getTaskType(), activity, task.getResolution(), task.getAgent());
        String reason = task.getResolution();
        workflowService.completeActivity(activity.getId(), Resolutions.EXPIRE, reason);
    }
}
